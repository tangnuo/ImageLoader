...
public class ImageCacheUtil implements ImageLoader.ImageCache {
    ...
    //路径
    private static final String CACHE_FOLDER_NAME = "ImageLoader_caowj";

    private final String TAG = ImageCacheUtil.class.getSimpleName();
    ...
    public ImageCacheUtil() {
        ...
        try {
            mDiskLruCache = DiskLruCache.open(getDiskCacheDir(MyApplication.Companion.getContext(), CACHE_FOLDER_NAME),
                    getAppVersion(MyApplication.Companion.getContext()), 1, DISKMAXSIZE);
        } catch (IOException e) {
            e.printStackTrace();
        }
    }
